...
public class ColorUtil {
    ...
    public static float perceptualQuantizer(float v) {
        return 10000f * (float)Math.pow(Math.max(Math.pow(v, 1f/m2) - c1, 0)/(c2 - c3*Math.pow(v, 1f/m2)), 1f/m1);
    }

    public static float perceptualQuantizerInverse(float v) {
        return (float)Math.pow((c1 + c2 + Math.pow(v, m1))/(1 + c3 + Math.pow(v, m1)), m2);
    }
    ...
    public static float[] delinearizeSrgb(float[] arr) {
        return new float[]{
    ...
    public static float[] clamp(float[] arr) {
        return new float[]{
                Math.max(0, Math.min(1, arr[0])),
                Math.max(0, Math.min(1, arr[1])),
                Math.max(0, Math.min(1, arr[2]))
        };
    }

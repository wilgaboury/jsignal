...
public class KotlinAnonymousClassPatchPlugin {
    private static AgentLogger LOGGER = AgentLogger.getLogger(KotlinAnonymousClassPatchPlugin.class);
    ...
    private static Map<ClassLoader, Map<String, KotlinAnonymousClassInfos>> anonymousClassInfosMap = new WeakHashMap<>();
    ...
    public static CtClass patchAnonymousClass(ClassLoader classLoader, ClassPool classPool, String className, Class original)
        ...
        String mainClass = javaClass.replaceAll("(\\$[a-zA-Z0-9]+)?(\\$\\d+)+$", "");

        // skip synthetic classes
        if (classPool.find(className) == null)
            return null;

        KotlinAnonymousClassInfos info = getStateInfo(classLoader, classPool, mainClass);
...
//        CtClass ctClass = classPool.get(javaClassName);
     ...
     */
    private static synchronized KotlinAnonymousClassInfos getStateInfo(ClassLoader classLoader, ClassPool classPool, String className) {
        Map<String, KotlinAnonymousClassInfos> classInfosMap = anonymousClassInfosMap.computeIfAbsent(classLoader, k -> new HashMap<>());

        KotlinAnonymousClassInfos infos = classInfosMap.get(className);
        ...
        if (infos == null || !infos.isCurrent(classPool)) {
            ...
            infos = new KotlinAnonymousClassInfos(classPool, className);
            infos.mapPreviousState(new KotlinAnonymousClassInfos(classLoader, className));
